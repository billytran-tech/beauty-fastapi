...
auth_custom = Auth0(domain=env.auth0_domain,
                    api_audience=env.auth0_api_audience, auth0user_model=CustomAuth0User)
auth_guest = Auth0(domain=env.auth0_domain,
                   api_audience=env.auth0_api_audience, auto_error=False)
app = FastAPI()
...
def get_malformed_token(token: str) -> str:
    ...
    bad_payload_encoded = base64.b64encode(
        bad_payload_str.encode()).decode().replace('=', '')
...
def get_missing_kid_token(token: str) -> str:
    ...
    bad_header_encoded = base64.b64encode(
        bad_header_str.encode()).decode().replace('=', '')
...
def test_public():
    ...
    # should be 401, see https://github.com/tiangolo/fastapi/pull/2120
    assert resp.status_code == 403, resp.text
...
def test_m2m_app():
    ...
    resp2 = client.get(
        '/also-secure-2', headers=get_bearer_header(access_token))
    assert resp2.status_code == 200, resp2.text

    user = Auth0User(**resp.json())
    assert env.auth0_test_permission in user.permissions
    # auth0 cannot provide an email because the end user is a machine
    ...
    resp = client.get('/secure-custom-user',
                      headers=get_bearer_header(access_token))
    assert resp.status_code == 200, resp.text
    ...
    resp = client.get(
        '/guest', headers=get_bearer_header(env.auth0_wrong_tenant_token))
    assert resp.status_code == 200, resp.text
    assert resp.json()['message'] == 'guest', resp.json()
...
def test_token():
    resp = client.get(
        '/secure', headers=get_bearer_header(env.auth0_expired_token))
    ...
    ) and 'header' not in error_detail.lower(), error_detail
    ...
    resp = client.get(
        '/secure', headers=get_bearer_header(env.auth0_wrong_tenant_token))
    assert resp.status_code == 401, resp.text
    error_detail = resp.json()['detail']
